...
func BenchmarkNewVsOldAPI(b *testing.B) {
	...
	b.Run("NewAPI_Email_Validation", func(b *testing.B) {
		schema := Email().WithEmailMessage("Invalid email")
	...
	b.Run("NewAPI_Number_Validation", func(b *testing.B) {
		schema := Number().Min(0).Max(100).Integer().Required()
		testNum := 42
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
...
func BenchmarkComplexSchemas(b *testing.B) {
	...
	userSchema := Object(map[string]interface{}{
		...
		"password": String().
			...
			Required(),
		"age": Number().Min(13).Max(120).Integer().Optional(),
		"interests": Array(String().Min(1)).MinItems(0).MaxItems(10).Optional(),
		"settings": Object(map[string]interface{}{
...
func BenchmarkArrayValidation(b *testing.B) {
	schema := Array(String().Min(1).Max(100)).
...
func BenchmarkStateTransitions(b *testing.B) {
	...
	b.Run("StringBuilder_To_Optional", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			schema := String().Min(3).Max(50).Optional().Default("default")
	...
	b.Run("NumberBuilder_To_Optional", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			schema := Number().Min(0).Max(100).Optional().Default(0.0)
			_ = schema.Validate(nil)
		}
	})
}
...
func BenchmarkErrorKeyPerformance(b *testing.B) {
	...
	b.Run("ErrorKeys_Method_Based", func(b *testing.B) {
		schema := String().Min(5).Required().
			WithMessage(Errors.MinLength(), "Too short").
			WithMessage(Errors.Required(), "Required")
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = schema.Validate(testValue)
		}
	...
	b.Run("ErrorKeys_Constants", func(b *testing.B) {
		schema := String().Min(5).Required().
			WithMessage(ErrMinLength, "Too short").
			WithMessage(ErrRequired, "Required")
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = schema.Validate(testValue)
		}
	...
	b.Run("ErrorKeys_Convenience_Methods", func(b *testing.B) {
		schema := String().Min(5).WithMinLengthMessage("Too short").
...
func BenchmarkMemoryAllocations(b *testing.B) {
	b.Run("String_Schema_Creation", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = String().Min(3).Max(50).Required()
		}
	})
	...
	b.Run("Array_Schema_Creation", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = Array(String().Min(1)).MinItems(1).MaxItems(10).Required()
		}
	})
}
...
func BenchmarkPreConfiguredValidators(b *testing.B) {
	...
	b.Run("PreConfigured_PositiveNumber", func(b *testing.B) {
		schema := PositiveNumber().Required()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = schema.Validate(42)
		}
...
func BenchmarkValidationFailures(b *testing.B) {
	...
	b.Run("String_Pattern_Failure", func(b *testing.B) {
		schema := String().Pattern(`^[a-zA-Z]+$`).Required()
	...
	b.Run("Number_Range_Failure", func(b *testing.B) {
		schema := Number().Min(0).Max(100).Required()
		testValue := -5
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = schema.Validate(testValue)
	...
	b.Run("Array_MinItems_Failure", func(b *testing.B) {
		schema := Array(String()).MinItems(5).Required()
...
func BenchmarkTypeConversions(b *testing.B) {
	...
	b.Run("Array_Type_Check", func(b *testing.B) {
		arraySchema := Array(String()).Required()
	...
	b.Run("Map_Type_Check", func(b *testing.B) {
		objectSchema := Object(map[string]interface{}{
